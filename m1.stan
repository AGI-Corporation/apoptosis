...
transformed parameters {
  ...
  vector[N] err;
  vector[C] log_kq = qconst + append_row(0, dq_free)[design] + cq;
  vector[C] log_td = tconst + append_row(0, dt_free)[design] + cd;
  vector[C] log_kd = dconst + append_row(0, dd_free)[design] + ct;
  {
    ...
    for (n in 1 : N) {
      ...
      err[n] = exp(mu_err + b_err * fmin(0, log(yhat[n] / 0.3)));
    }
  }
}
model {
