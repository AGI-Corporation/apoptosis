...
transformed parameters {
  ...
  // Hoist exponentiations for clone-level parameters
...
generated quantities {
  ...
  // In Stan, parameters from transformed parameters are in scope in generated quantities.
  // Re-declaring them is an error.
  for (n in 1:N_test){
    int r = replicate_test[n];
    int c = clone[r];
    real yhat_test =
      yt(t_test[n], R0[r], mu, kq_vec[c], td_vec[c], kd_vec[c]);
    real err_test = exp(mu_err + b_err * fmin(0, log(yhat_test / 0.3)));
