...
transformed parameters {
  ...
  vector<lower=0>[C] kq_vec = exp(log_kq);
  vector<lower=0>[C] td_vec = exp(log_td);
  vector<lower=0>[C] kd_vec = exp(log_kd);
  {
    vector[N] x_small;
    for (n in 1:N){
        int r = replicate[n];
        ...
        x_small[n] = yhat[n] > 0.3 ? 0.0 : log(yhat[n] / 0.3);
    }
...
generated quantities {
  ...
  {
    vector[C] kq_vec_local = exp(log_kq);
    vector[C] td_vec_local = exp(log_td);
    vector[C] kd_vec_local = exp(log_kd);
    for (n in 1:N_test){
      int r = replicate_test[n];
      int c = clone[r];
      real yhat_test =
        yt(t_test[n], R0[r], mu, kq_vec_local[c], td_vec_local[c], kd_vec_local[c]);
