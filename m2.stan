...
transformed parameters {
  ...
  // Hoist exponentiations out of the loop
...
generated quantities {
  ...
  // Hoist exponentiations
  vector[D] exp_tconst_dt = exp(tconst + dt);
  vector[D] exp_dconst_dd = exp(dconst + dd);

  vector[D] avg_delay = exp_tconst_dt + inv(exp_dconst_dd);
  vector[D] tauD = exp_tconst_dt;
  vector[D] k_d = exp_dconst_dd;

  // Use hoisted parameters from transformed parameters
  for (n in 1:N_test){
