...
generated quantities {
  ...
  vector[C] kq_vec_test = exp(log_kq);
  vector[C] td_vec_test = exp(log_td);
  vector[C] kd_vec_test = exp(log_kd);
  for (n in 1:N_test){
    int r = replicate_test[n];
    int c = clone[r];
    real yhat_test =
      yt(t_test[n], R0[r], mu, kq_vec_test[c], td_vec_test[c], kd_vec_test[c]);
