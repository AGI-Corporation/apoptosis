...
/**
 * Optimized analytic solution for total cell density.
 * Simplified algebraically to reduce the number of exp() calls from 6 to 1 (t < td)
 * or 3 (t >= td). Added a 1e-9 floor for numerical stability in log-likelihoods.
 */
real yt(real t, real R0, real mu, real kq, real td, real kd){
  real sm = mu - kq;
  if (t < td) {
    real esmt = exp(sm * t);
    real out = R0 * esmt + kq * R0 / sm * (esmt - 1);
    return out > 1e-9 ? out : 1e-9;
  } else {
    real esmt = exp(sm * t);
    real esmtd = exp(sm * (t - td));
    real ekdt_rel = exp(kd * (td - t));
    real out = R0 * esmt + kq * R0 / sm * (esmt - esmtd)
               + kq * R0 / (sm + kd) * (esmtd - ekdt_rel);
    return out > 1e-9 ? out : 1e-9;
  }
}
