 ...
 *
 * This function provides an optimized analytic solution to the ODE system.
 * By algebraically simplifying the solution, we reduce the number of expensive
 * exp() calls from 6 to 3 for t >= td, and from 3 to 1 for t < td.
 */
real yt(real t, real R0, real mu, real kq, real td, real kd){
  real sm = mu - kq;
  if (t < td) {
    // Before delay td: yt = R0 * exp(sm * t) + kq * R0 / sm * (exp(sm * t) - 1)
    // Simplified: yt = R0 * (mu * exp(sm * t) - kq) / sm
    return R0 * (mu * exp(sm * t) - kq) / sm;
  } else {
    // After delay td:
