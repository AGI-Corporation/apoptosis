 ...
 * Optimized analytic solution to the ODE system.
 * Reduces exp() calls and uses expm1() for stability.
 */
real yt(real t, real R0, real mu, real kq, real td, real kd){
  real sm = mu - kq;
  if (t < td) {
    // Simplified: R0 * (mu/sm * expm1(sm * t) + 1)
    return R0 * (mu / sm * expm1(sm * t) + 1);
  } else {
    // Simplified: R0 * ( mu/sm * exp(sm * t) - kq/(sm+kd) * ( kd/sm * exp(sm*(t-td)) + exp(-kd*(t-td)) ) )
    return R0 * ( mu / sm * exp(sm * t) - (kq / (sm + kd)) * ( (kd / sm) * exp(sm * (t - td)) + exp(-kd * (t - td)) ) );
  }
}
...
real Rt_num(real t, real R0, real sm) {
  return R0 * exp(sm * t);
}
