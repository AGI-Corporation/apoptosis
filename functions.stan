...
real yt(real t, real R0, real mu, real kq, real td, real kd) {
  real sm = mu - kq;
  real exp_smt = exp(sm * t);
  if (t < td) {
    return R0 * exp_smt * (1 + kq / sm) - kq * R0 / sm;
  } else {
    real exp_smtd = exp(sm * td);
    real exp_smt_minus_td = exp_smt / exp_smtd;
    real exp_kd_t_minus_td = exp(-kd * (t - td));
    return R0 * exp_smt + kq * R0 / sm * (exp_smt - 1)
           - kq * R0 / sm * (exp_smt_minus_td - 1)
           + kq * R0 / (sm + kd) * (exp_smt_minus_td - exp_kd_t_minus_td);
  }
}
...
vector dsdt(real t, vector y, real R0, real sm, real kq, real td, real kd) {
  vector[4] flux = [(sm + kq) * y[1], kq * y[1],
                    t < td ? 0 : kq * R0 * exp(sm * (t - td)), kd * y[3]]';
