...
real yt(real t, real R0, real mu, real kq, real td, real kd) {
  /*
     Analytically solve the ODE system to find total cell density.
     Optimized to minimize exp() calls:
     - t < td: 1 exp() call
     - t >= td: max 3 exp() calls
  */
  real sm = mu - kq;
  real val;
  if (t < td) {
    ...
    val = R0
          * (mu / sm * exp(sm * t) - kq * kd / (sm * (sm + kd)) * exp(sm * (t - td))
...
real yt_num(real t, real R0, real mu, real kq, real td, real kd) {
  real sm = mu - kq;
  array[1] vector[3] sol = ode_rk45(dsdt, [R0, 0, 0]', 0, {t}, R0, sm, kq, td,
                                    kd);
  real out = sum(sol[1]);
